...
extension String {
    ...
    internal func normalize(options: NormalizeOptions = .default) -> String {
        ...
        if options.contains(.preserveSiblingNewlines) {
            ...
            result = result.replacingOccurrences(of: "}\\s*Button", with: "}\nButton", options: .regularExpression)
            
            // Add newline after method chaining
            result = result.replacingOccurrences(of: "\\.foregroundColor\\([^)]*\\)\\s*}", with: ".foregroundColor($1)\n}", options: .regularExpression)
            
            // Normalize Task closure formatting
            result = result.replacingOccurrences(of: "Task\\s*{\\s*@MainActor", with: "Task { @MainActor", options: .regularExpression)
