    ...
    public var syntax: SyntaxProtocol {
        ...
        let paramList = parameters.map { param in
            TupleTypeElementSyntax(
                type: param.type.map { IdentifierTypeSyntax(name: .identifier($0)) } ?? IdentifierTypeSyntax(name: .identifier("Any"))
            )
        }
    ...
    private func buildAttributeList(from attributes: [AttributeInfo]) -> AttributeListSyntax {
        ...
        let attributeElements = attributes.enumerated().map { index, attributeInfo in
            ...
            let atSign = index == 0 ? 
                ...
                TokenSyntax.atSignToken(leadingTrivia: .space)
...
extension ClosureType: TypeRepresentable {
    public var typeSyntax: TypeSyntax {
        ...
        let paramList = parameters.map { param in
            TupleTypeElementSyntax(
                type: param.type.map { IdentifierTypeSyntax(name: .identifier($0)) } ?? IdentifierTypeSyntax(name: .identifier(param.name))
            )
        }
        // Build return clause
        var returnClause: ReturnClauseSyntax?
