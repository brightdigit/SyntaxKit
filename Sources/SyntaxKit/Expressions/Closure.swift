...
public struct Closure: CodeBlock {
    ...
    public var syntax: SyntaxProtocol {
        // Capture list
        let captureClause: ClosureCaptureClauseSyntax? = capture.isEmpty ? nil : ClosureCaptureClauseSyntax(
        ...
        let signature: ClosureSignatureSyntax? = (parameters.isEmpty && returnType == nil && capture.isEmpty && attributes.isEmpty) ? nil : ClosureSignatureSyntax(
            attributes: attributes.isEmpty ? AttributeListSyntax([]) : AttributeListSyntax(
                attributes.enumerated().map { idx, attr in
                    AttributeListSyntax.Element(
                        AttributeSyntax(
                            atSign: .atSignToken(),
                            attributeName: IdentifierTypeSyntax(name: .identifier(attr.name), trailingTrivia: (capture.isEmpty || idx != attributes.count - 1) ? Trivia() : .space),
                            leftParen: nil,
                            arguments: nil,
                            rightParen: nil
                        )
            ...
            parameterClause: parameters.isEmpty ? nil : .parameterClause(ClosureParameterClauseSyntax(
            ...
            returnClause: returnType == nil ? nil : ReturnClauseSyntax(
            ...
            body.compactMap {
                if let decl = $0.syntax.as(DeclSyntax.self) {
                    ...
                    if let exprBlock = paramExp.value as? ExprCodeBlock {
                        return CodeBlockItemSyntax(item: .expr(exprBlock.exprSyntax)).with(\.trailingTrivia, .newline)
