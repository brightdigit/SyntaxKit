...
# Ask SPM for the canonical bin dir rather than globbing `.build/<triple>/`,
# which picks an arbitrary match (`head -1`) when several host triples exist.
BUILD_DIR="$(swift build "${SWIFT_CONFIG_FLAGS[@]}" --show-bin-path 2>/dev/null)"
if [[ -z "$BUILD_DIR" || ! -d "$BUILD_DIR" ]]; then
  echo "Could not locate ${CONFIG} build dir via 'swift build --show-bin-path'" >&2
  exit 1
fi
